    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double vx, vy, front_car_s;

          double lowest_s = 10000;
          int front_nearest = -1;
          double front_s, front_d;
          
          for(int i = 0; i < sensor_fusion.size(); i++){ //find nearest ahead
            front_d = sensor_fusion[i][6];
            front_s = (double)sensor_fusion[i][5] - car_s;
			//cout << front_s << endl;
            if(front_d > (4*curr_lane) && front_d < (4+4*curr_lane) && (front_s < lowest_s) && (front_s > 0)){
              front_nearest = i;
              lowest_s = front_s;
          ...
          if(front_nearest > -1){
            vx = sensor_fusion[front_nearest][3];
            vy = sensor_fusion[front_nearest][4];
          ...
          if (behavior_sm == 1){ //in PLC mode, loop through sensor fusion data to find costs of lane changes
            ...
            for (int i = 0; i < sensor_fusion.size(); i++){
              ...
              if ((side_d < (4 + 4*(curr_lane-1))) && (side_d > (4*(curr_lane-1)))){
                //car is on left
                if (side_s < nearest_left_s){
                  nearest_left_s = side_s;
                  nearest_left = i;
                }
                if (side_s < front_car_s && side_s > -4){ //count cars in the way of changing lanes
                  num_left += 1;
                }
            ...
            left_cost = num_left + 0.01;
            right_cost = num_right + 0.01; //todo
            ...
            //as we iterate through nearby vehicles
            //if curr_lane < 2:
            //iff sm = 1, ie following, iterate through left lane vehicles, find nearest, and count how many between followed car and current - 20
            //if curr_lane > 0:
            //same deal for right lane vehicles
            //compute cost for each side based on distance to nearest vehicle, velocity of nearest, current velocity
          ...
          double increment;
          //cout << behavior_sm << "sm" << endl;
          //cout << following_active << "fa" << endl;
          ...
          also look for nearby cars < s_front, > s_ego - 3
